	    ...
	    try {
			for(ItemInfo info : item.info()) {
				if(info instanceof ItemInfo.Contents) { // Draw content bar
					...
					String contentName = ((ItemInfo.Contents) info).contentName;
					if(((ItemInfo.Contents) info).seeds) {
						col = new Color(128, 90, 48, 255);
					} else if(((ItemInfo.Contents) info).contentName.equals("Milk") || contentName.equals("Goatsmilk") ) {
						col = Color.white;
	    	...
	    	// Ignore if info res not loaded yet
		}
	} else {
